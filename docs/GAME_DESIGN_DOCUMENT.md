...
**关系要求表**（v1.2 更新，修复实习生无法满足监理要求的问题）:

| 职级晋升 | 核心关系要求 | 要求类型 | 说明 |
|---------|-------------|---------|------|
| 实习生 → 助理工程师 | 甲方≥50 **或** 劳务队≥50 | any（任一） | 实习生只能维护甲方和劳务队 |
| 助理工程师 → 工程师 | 监理≥60、甲方≥60、劳务队≥60 | all（全部） | 助理工程师解锁了监理 |
| 工程师 → 高级工程师 | 设计院≥60 | all | 工程师解锁了设计院 |
| 高级工程师 → 项目经理 | 监理≥65、劳务队≥60 | all | 高工需要协调监理和劳务 |
| 项目经理 → 项目总监 | 甲方≥70、政府≥60 | all | 总监级需要甲方和政府资源 |
| 项目总监 → 合伙人 | 所有关系≥75 | all | 合伙人需要全方位资源 |
...
```typescript
// 基础准确率
基础准确率 = 50 + workAbility / 2  // 50%-100%

// 应用特殊效果（如设计优化方案）
最终准确率 = 基础准确率 + pricePredictionBonus

// 示例
workAbility = 50 → 基础准确率 75%
获得"设计优化方案"后 → 最终准确率 75% + 50% = 125% → 限制为 100%
```

**趋势判定**:
```typescript
随机值 = random(0, 100)
趋势判定 = 随机值 < min(准确率, 100) ? true : false
```

**价格范围**:
```typescript
区间宽度 = (100 - 准确率) / 100 × 20%
价格区间 = [基准价格 × (1 - 区间), 基准价格 × (1 + 区间)]
```
...
| 关系值 | 预警级别 | 提示信息 | 动画 |
|-------|---------|---------|------|
| ≥ 80 | 良好提示 | "✨ 关系良好，享有经济收益加成" | 无 |
| ≤ 40 | 红色警告 | "⚠️ 关系紧张！负面事件风险增加" | 闪烁 |
| ≤ 30 | 橙色警告 | "🚨 危险！可能触发严重负面事件" | 无 |

**风险提示说明**：
- **关系 ≤ 40**：触发负面事件概率为 25%-100%（见 7.4.1 节）
- **关系 ≤ 30**：可能触发中度到重度负面事件
- **关系 ≥ 80**：解锁经济收益事件（见 7.7 节）
...
#### 1. 工作能力价格影响机制
- **价格波动控制**：工作能力现在真正影响市场价格波动
  - workAbility 0 → ±15% 波动（完全随机）
  - workAbility 50 → ±10% 波动
  - workAbility 100 → ±5% 波动（高度稳定）
- 实现位置：`generateNextQuarterPrices` 函数
- 详见：6.2 节

#### 2. 关系要求修复与对齐
- **问题**：原配置中实习生→助理工程师需要监理≥60，但实习生没有监理关系
- **修复**：调整各职级关系要求，只使用已解锁的关系
  - 实习生 → 助理工程师：甲方≥50 **或** 劳务队≥50（any）
  - 助理工程师 → 工程师：监理≥60、甲方≥60、劳务队≥60（all）
  - 工程师 → 高级工程师：设计院≥60
  - 高级工程师 → 项目经理：监理≥65、劳务队≥60
  - 项目经理 → 项目总监：甲方≥70、政府≥60
- 实现位置：`shared/types/game.ts` 职级配置、`gameStoreNew.ts` checkPromotion 函数
- 详见：3.2 节关系要求表

#### 3. Store 统一
- **问题**：存在 `gameStore.ts`（旧）和 `gameStoreNew.ts`（新）双 store 并存
- **解决**：所有页面统一使用 `gameStoreNew`
  - `Game.tsx` - 事件阶段页面
  - `Result.tsx` - 游戏结束结算页面
  - `QuarterlySettlement.tsx` - 季度结算页面
  - `StrategyPhase.tsx` - 策略阶段页面
- 移除临时属性映射（tempWorkAbility、tempLuck）
- 实现位置：`frontend/src/pages/*.tsx`

#### 4. 特殊效果系统完善
- **新增状态**：`pricePredictionBonus`、`storageFeeDiscount`
- **设计优化方案**：技术交流（设计院）workAbility≥60 时 20% 获得，价格预测准确率 +50%
- **政策解读**：政策学习（政府）workAbility≥70 时 10% 获得，仓储费 -50%
- **效果持续时间**：本季度有效，季度开始时失效
- **UI 显示**：顶部状态栏显示当前生效的特殊效果
- 实现位置：`shared/types/game.ts`、`gameStoreNew.ts`、`TopStatusBar.tsx`

#### 5. 关系预警系统增强
- **UI 显示**：关系卡片下方显示预警提示
- **预警等级**：
  - 关系 ≥ 80：✨ 绿色提示"关系良好，享有经济收益加成"
  - 关系 ≤ 40：⚠️ 红色警告"关系紧张！负面事件风险增加"
  - 关系 ≤ 30：🚨 橙色警告"危险！可能触发严重负面事件"
- **动画效果**：红色警告使用闪烁动画（animate-pulse）
- 实现位置：`RelationsPage.tsx`

#### 6. Bug 修复
- **项目完成后状态重置 Bug**：添加 `projectCompletedThisQuarter` 标志
  - 问题：项目完成后下季度开始事件会覆盖重置后的状态
  - 解决：项目完成后忽略下季度开始事件的项目效果
- 实现位置：`gameStoreNew.ts`

**实现文件**:
- `shared/types/game.ts` - 职级关系要求配置修复
- `frontend/src/store/gameStoreNew.ts` - 关系检查、特殊效果、项目重置
- `frontend/src/components/TopStatusBar.tsx` - 特殊效果显示
- `frontend/src/pages/RelationsPage.tsx` - 关系预警显示
- `frontend/src/pages/*.tsx` - Store 统一
...
#### 1. 项目统计显示
- **UI 位置**：顶部状态栏（TopStatusBar）
- **显示内容**：
  - 已完成项目总数
  - 优质项目数量（质量≥90，带⭐图标）
- **视觉设计**：绿色渐变背景，清晰展示玩家成就
- 实现位置：`TopStatusBar.tsx`
...
- **通知内容**：
  ...
  - 优质项目对晋升的帮助提示
- **交互**："太棒了！"按钮关闭通知
- **通知状态**：`qualityProjectJustCompleted`
- 实现位置：`Game.tsx`、`gameStoreNew.ts`
...
- **重要性**：
  ...
  - 项目总监晋升：需要 5 个优质项目
  - 合伙人晋升：需要 10 个优质项目
- 详见：3.1 节职级表、11.3 节游戏结束统计

**实现文件**:
- `shared/types/game.ts` - 添加 `qualityProjectJustCompleted` 状态
- `frontend/src/store/gameStoreNew.ts` - 通知逻辑、dismiss 函数
- `frontend/src/components/TopStatusBar.tsx` - 项目统计显示
- `frontend/src/pages/Game.tsx` - 优质项目完成通知弹窗
