...
4. **离职机制**
   - 士气 < 30：显示黄色警告
   - 士气 < 20：显示红色警告，下季度 30% 概率离职

5. **问题生成**
   - 修复：在 nextQuarter 中调用 generateTeamIssue
   - 每季度 30% 概率生成团队问题

6. **工资扣除**
   - finishQuarter 中计算并扣除所有成员工资
...
- `frontend/src/store/gameStoreNew.ts`
  - 修改 TEAM_PROJECT：添加职业效果计算
  - 修改 TEAM_TRAINING：添加技能提升概率
  - 修改 updateTeamEfficiency：考虑技能值
  - 修改 finishQuarter：添加团队工资、士气衰减、问题处理、离职
  - 修改 nextQuarter：调用 generateTeamIssue
- `frontend/src/pages/TeamPage.tsx`
  - 添加职业功能说明弹窗（ROLE_INFO）
  - 添加士气警告显示
  - 添加士气进度条
  - 添加"❓"帮助按钮
...
### 团队系统重构：所有团队功能移至"团队"Tab

**问题描述**：
- 团队功能分散在"行动"和"团队"两个 Tab 中，用户体验不清晰
- 用户需要在不同页面寻找团队相关功能

**解决方案**：

1. **重构 TeamPage.tsx**（团队 Tab）
   - 添加行动点显示
   - 添加消息提示 toast
   - 添加"团队行动"区域（团队项目、团队培训）
   - 完善招募成员功能（带行动点检查）
   - 完善解决问题功能（带领导力检查）

2. **简化 ActionsPage.tsx**（行动 Tab）
   - 移除"团队行动"区域
   - 移除招募弹窗和问题弹窗
   - 团队状态卡片改为可点击，跳转到团队 Tab
   - 只保留基础行动（做项目、接私活、偷工减料、休息）和属性训练

**涉及文件**：
- `frontend/src/pages/TeamPage.tsx` - 重构，添加完整团队功能
- `frontend/src/pages/ActionsPage.tsx` - 简化，移除团队功能
...
**问题描述**：
- 后期阶段（项目经理及以上）的团队系统玩法不够完善
- 玩家需要手动触发招募/解决问题等操作，UI交互不明确
- 团队项目、团队培训功能需要实现

**解决方案**：

1. **添加 TEAM_TRAINING 行动类型**
   - 在 `ActionType` 枚举中添加 `TEAM_TRAINING`
   - 在 `ACTIONS` 配置中添加团队培训：花费 5000，提升士气 +10，领导力 +3

2. **重构 `doAction` 函数返回值**
   - 添加 `requiresUI` 字段，用于标识需要 UI 交互的操作
   - `RECRUIT` 返回 `requiresUI: 'recruit'` 触发招募弹窗
   - `RESOLVE_ISSUE` 返回 `requiresUI: 'resolveIssue'` 触发问题弹窗
   - `TEAM_PROJECT` 直接执行，带团队效率加成，30% 概率获得领导力
   - `TEAM_TRAINING` 直接执行，提升士气和领导力

3. **完善 ActionsPage.tsx UI**
   - 添加团队状态卡片（显示成员数、效率、领导力、待处理问题）
   - 添加招募成员弹窗（选择招募类型）
   - 添加解决问题弹窗（显示待处理问题和所需领导力）
   - 添加消息提示 toast（3秒自动消失）

4. **创建调试页面 Debug.tsx**
   - 需要 admin 密码登录
   - 快捷跳转到后期阶段（项目经理）
   - 自定义状态调整（职级、现金、健康、声誉、阶段、季度）
   - 添加测试团队成员

**涉及文件**：
- `shared/types/game.ts` - 添加 TEAM_TRAINING 到 ActionType 枚举
- `frontend/src/data/constants.ts` - 添加 TEAM_TRAINING 配置
- `frontend/src/store/gameStoreNew.ts` - 重构 doAction 返回值，添加 setDebugState 和 addTeamMember
- `frontend/src/pages/ActionsPage.tsx` - 完善团队 UI（状态卡片、弹窗、消息提示）
- `frontend/src/pages/Debug.tsx` - 新建调试页面
...
**特殊改动点**：
- doAction 返回 ActionResult 接口，新增 requiresUI 字段支持 UI 交互
- 招募和解决问题不再在 doAction 中直接执行，而是返回信号让 UI 层处理
- 调试页面方便快速测试后期团队系统
...
**测试目的**：验证从7级（实习生→助理工程师→工程师→高级工程师→项目经理→项目总监→合伙人）到6级（实习生→工程师→高级工程师→项目经理→项目总监→合伙人）的职级系统重构完整性和正确性。
...
2. **RANK_CONFIGS 配置验证**
   - ✅ 工程师：20万净资产，1项目，声誉45
   - ✅ 高级工程师：100万净资产，6项目，声誉65，1优质项目
   - ✅ 项目经理：400万净资产，12项目，声誉80，3项目
   - ✅ 项目总监：1200万净资产，22项目，声誉90，5优质项目
   - ✅ 合伙人：4000万净资产，35项目，声誉95，8优质项目

3. **RANK_DISPLAY 显示配置验证**
   - ✅ 没有助理工程师条目
   - ✅ 图标正确分配：
     - 实习生：🎓
     - 工程师：👨‍🔧
...
5. **遗留引用清理验证**
   - ✅ 前端代码：0个 `ASSISTANT_ENGINEER` 引用
   - ✅ 后端代码：0个 `ASSISTANT_ENGINEER` 引用（修复了 leaderboard.ts）
   - ✅ Shared 代码：0个 `ASSISTANT_ENGINEER` 引用

6. **后端排行榜 API 修复**
   - ✅ 移除了所有 `ASSISTANT_ENGINEER` 的 CASE WHEN 分支
   - ✅ 更新了职级权重配置
   - ✅ 添加了"6级职级系统"注释说明

**涉及文件**：
- `frontend/src/data/constants.ts` - 更新 RANK_DISPLAY 图标和关系解锁提示
- `shared/types/game.ts` - 更新 RANK_CONFIGS 配置（已在之前提交中完成）
- `backend/src/api/leaderboard.ts` - 移除 ASSISTANT_ENGINEER 引用，更新职级权重

**Review 状态**：✅ 自测通过，待人工测试验证

**特殊改动点**：
- 后端排行榜 API 需要同步清理遗留的助理工程师引用
- 关系解锁提示文案从"晋升助理工程师解锁"改为"晋升工程师解锁"
- 职级图标进行了重新分配以匹配新的6级系统

---

## 2026-01-30

### 功能完善：LLM 事件生成系统 RSS 源更新

**问题描述**：
- 用户询问 LLM 事件生成系统的两种生成方式（news 和 creative）的运行状态
- 调查发现数据库中只有 20 个 creative 类型事件，没有 news 类型事件
- 进一步测试发现原配置的 7 个 RSS 源全部失效（404 错误、Feed 格式错误等）

**RSS 源测试结果**：
- 原有 7 个源：全部失效
  - 建筑时报、中国建筑新闻网、腾讯新闻、新华网、凤凰网资讯、财经网房产、科技日报
- 测试 27 个候选源，找到 12 个可用源

**解决方案**：
更新 `backend/src/config/rss-sources.ts` 配置，替换为经过测试验证的可用 RSS 源：

**新的 RSS 源配置（7个）**：
1. **人民网-时政** - 100条/次，综合新闻，权重 1.0
2. **人民网-财经** - 100条/次，财经新闻，权重 1.2
3. **人民网-社会** - 100条/次，社会新闻，权重 1.0
4. **人民网-科技** - 100条/次，科技新闻，权重 0.8
5. **中国新闻网-国内** - 30条/次，国内新闻，权重 1.0
6. **中国新闻网-财经** - 30条/次，财经新闻，权重 1.2
7. **中国新闻网-社会** - 30条/次，社会新闻，权重 1.0

**测试验证**：
- ✅ RSS 抓取测试：6/7 源成功抓取，共获取 40 条新闻
- ✅ LLM 事件生成测试：成功从真实新闻生成 5 个游戏事件
- ✅ 数据库保存验证：新闻事件成功存储到 dynamic_events 表
- ✅ 事件质量评分：0.80-0.95（高质量）

**生成的新闻事件示例**：
- "村厕改造任务" - 项目组接村厕改造试点
- "农田基建项目" - 高标准农田项目含道路与灌溉渠
- "农普基建核查" - 农业普查需核查乡村土木设施
- "军建新规发布" - 军事设施建设条例施行
- "考点周边整改" - 高考临近考点周边土木整改

**系统状态确认**：
- ✅ Creative 类型事件：20 个（LLM 创意生成）
- ✅ News 类型事件：8 个（基于真实新闻生成）
- ✅ 定时任务调度器：正常运行
  - 每日凌晨 3:00：新闻抓取 + 事件生成
  - 每日凌晨 4:00：清理过期事件
  - 每 2 小时：补充事件检查
...
**涉及文件**：
- `backend/src/config/rss-sources.ts` - 更新 RSS 源配置
- `backend/test-rss-sources.ts` - 新增：RSS 源批量测试脚本
- `backend/test-rss-sources-2.ts` - 新增：第二批 RSS 源测试脚本
- `backend/test-new-rss-config.ts` - 新增：新配置验证脚本
- `backend/test-full-news-generation.ts` - 新增：完整流程测试脚本
- `backend/test-news-generation.ts` - 新增：事件生成测试脚本
